...
public class GetTypeCardFromDeckWithoutAttackAbility : Ability
{
    ...
    private void DisplayOkMessage(Transform canvas)
    {
        var config = new MessageBoxConfig(
            ...
            LocalizationService.GetLocalizedValue(LocalizationKeys.INFORMATION_NO_CARD_GET_FROM_DECK_MESSAGE),
            showOkButton: true
        );
    ...
    public override void ApplyEffect(Transform canvas, PlayerCards playerCards, PlayerCards opponentPlayerCards)
    {
        ...
        if (hasCardInDeck)
        {
            var config = new MessageBoxConfig(
                ...
                string.Format(
                    LocalizationService.GetLocalizedValue(LocalizationKeys.QUESTION_GET_TYPE_CARD_MESSAGE),
                    type.ToName()
                ),
                ...
                positiveAction: () =>
                {
                    ...
                    var selectorConfig = new CardSelectorConfig(
                        LocalizationService.GetLocalizedValue(LocalizationKeys.CARDS_SELECTOR_TITLE_DEFAULT_CHOICE_CARD),
                        cards,
                        showNegativeButton: true,
                        showPositiveButton: true,
                        positiveAction: (card) =>
