...
namespace Lean.Localization
{
    ...
    public class LeanLocalization : MonoBehaviour
    {
        ...
        [FSA("DetectLanguage")] [SerializeField]
        private DetectType detectLanguage = DetectType.SystemLanguage;
        ...
        [FSA("DefaultLanguage")] [SerializeField] [LeanLanguageName]
        ...
        [FSA("saveLanguage")] [FSA("SaveLanguage")] [SerializeField]
        private SaveLoadType saveLoad = SaveLoadType.WhenChanged;
...
namespace Lean.Localization.Editor
{
    ...
    public class LeanLocalization_Editor : LeanEditor
    {
        ...
        protected override void OnInspector()
        {
            ...
            switch (tgt.DetectLanguage)
            {
                ...
                case LeanLocalization.DetectType.CurrentCulture:
                    EditorGUILayout.TextField("CurrentCulture",
                        System.Globalization.CultureInfo.CurrentCulture.ToString());
                    break;
                case LeanLocalization.DetectType.CurrentUICulture:
                    EditorGUILayout.TextField("CurrentUICulture",
        ...
        private void DrawTranslations()
        {
            ...
            BeginDisabled(string.IsNullOrEmpty(translationFilter) == true ||
                          LeanLocalization.CurrentTranslations.ContainsKey(translationFilter) == true);
            ...
            else
            {
                ...
                foreach (var pair in LeanLocalization.CurrentTranslations)
                {
                        ...
                        System.StringComparison.InvariantCultureIgnoreCase) >= 0)
                    {
                        var translation = pair.Value;
                        var rectT = Reserve();
                        var expand = EditorGUI.Foldout(new Rect(rectT.x, rectT.y, 20, rectT.height),
                        ...
                        if (expand == true)
                        {
                            ...
                            {
                                Warning("This translation is defined multiple times for the " + language +
                                        " language.");
                            }
                        }
                    }
        ...
        private void DrawLanguages()
        {
            ...
            BeginDisabled(string.IsNullOrEmpty(languagesFilter) == true ||
                          LeanLocalization.CurrentLanguages.ContainsKey(languagesFilter) == true);
            if (GUI.Button(rectC, "Add", EditorStyles.miniButton) == true)
            {
            ...
            if (LeanLocalization.CurrentLanguages.Count > 0 || string.IsNullOrEmpty(languagesFilter) == false)
            {
                ...
                if (total == 0)
                {
                    EditorGUILayout.HelpBox("No language with this name exists, click the 'Add' button to create it.",
                        MessageType.Info);
                }
            }
        }
        ...
        private void DrawTokens()
        {
            ...
            BeginDisabled(string.IsNullOrEmpty(tokensFilter) == true ||
                          LeanLocalization.CurrentTokens.ContainsKey(tokensFilter) == true);
            if (GUI.Button(rectC, "Add", EditorStyles.miniButton) == true)
            {
            ...
            if (LeanLocalization.CurrentTokens.Count > 0 || string.IsNullOrEmpty(tokensFilter) == false)
            {
                ...
                if (total == 0)
                {
                    EditorGUILayout.HelpBox("No token with this name exists, click the 'Add' button to create it.",
                        MessageType.Info);
                }
            }
        }
