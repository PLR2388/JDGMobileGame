            ...
            if (GUI.Button(right, "List") == true)
            {
                ...
                if (string.IsNullOrEmpty(property.stringValue) == false)
                {
                    if (exists == true)
                    {
                        ...
                        if (LeanLocalization.CurrentTranslations.TryGetValue(property.stringValue, out translation) ==
                            true)
                        {
                            foreach (var entry in translation.Entries)
                            {
                                var owner = entry.Owner;
                                menu.AddItem(new GUIContent("Select/" + entry.Language), false, () =>
                                {
                    ...
                    {
                        menu.AddItem(new GUIContent("Add: " + property.stringValue.Replace('/', '\\')), false, () =>
                        {
                            var phrase = LeanLocalization.AddPhraseToFirst(property.stringValue);
                ...
                {
                    menu.AddItem(new GUIContent(translationName), property.stringValue == translationName, () =>
                    {
